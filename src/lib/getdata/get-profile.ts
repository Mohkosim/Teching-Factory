...
export async function getCurrentUserProfile(): Promise<ProfileData | null> {
    ...
    return {
        ...
        kecamatan: user.smk?.kecamatan ?? null,
        kota: user.smk?.kota ?? null,
        kota_id: user.smk?.kota_id ?? null,
        kode_pos: user.smk?.kode_pos ?? null,
        latitude: user.smk?.latitude ?? null,  
        longitude: user.smk?.longitude ?? null, 
