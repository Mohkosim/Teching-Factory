// lib/utils/export-laporan-keuangan.ts
...
function writeSummaryBlock(
    ...
    sheet.getCell(rowIdx, 1).font = { bold: true, size: 11 };
    rowIdx++;
    ...
    rows.forEach(([label, value], i) => {
        ...
        const valueCell = sheet.getCell(rowIdx, 3);
...
function writeRingkasanSection(
    ...
    rowIdx = writeSummaryBlock(sheet, rowIdx, "RINGKASAN LABA KOTOR", [
    ...
    rowIdx = writeSummaryBlock(sheet, rowIdx, "RINGKASAN LABA BERSIH", [
        ["Laba Kotor", ringkasan.labaKotor],
...
function writeDetailTable(
    ...
    sheet.mergeCells(rowIdx, 1, rowIdx, LAST_COL);
    sheet.getCell(rowIdx, 1).value = "DETAIL TRANSAKSI";
    sheet.getCell(rowIdx, 1).font = { bold: true, size: 11 };
    rowIdx++;

    const headers = [
        "No", "No. Invoice", "Tanggal", "Pembeli/Pemasok", "Jurusan",
    ...
    transaksi.forEach((item, idx) => {
        const values: (string | number | null)[] = [
            idx + 1, item.noInvoice, item.tanggal, item.pembeliPemasok, item.jurusan,
