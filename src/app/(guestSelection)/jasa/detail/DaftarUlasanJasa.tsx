...
export default function DaftarUlasanJasa({
  reviews,
  jumlahReview,
}: {
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-400">
              Menampilkan {reviewsTerurut.length} dari {jumlahReview} review
            </p>
          </div>
                ...
                {r.fotos.length > 0 && (
                  <div className="mt-3 flex gap-2 overflow-x-auto">
                    {r.fotos.map((foto, i) => (
                      ...
                      >
                        <Image src={foto} alt={`Foto review ${i + 1}`} fill className="object-cover" />
                      </div>
                    ))}
                  </div>
                )}
