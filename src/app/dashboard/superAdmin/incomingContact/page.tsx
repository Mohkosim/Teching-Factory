...
import { Star, Trash2, Inbox, Eye } from "lucide-react";
...
const generateMessages = (): Message[] =>
    Array.from({ length: 20 }, (_, i) => ({
        ...
        preview: "Forem ipsum dolor sit amet ...",
        body: "Forem ipsum dolor sit amet, consectetur adipiscing elit. Etiam eu turpis molestie, dictum est a, mattis tellus. Sed dignissim, metus nec fringilla accumsan, risus sem sollicitudin lacus.",
        date: "26/08/2025",
        isRead: i % 3 === 0,
        isFavorite: i % 7 === 0,
        isDeleted: false,
...
export default function IncomingContact() {
    ...
    const markAsRead = () => {
        setMessages((prev) =>
            prev.map((m) => selected.includes(m.id) ? { ...m, isRead: true } : m)
        );
        setSelected([]);
    };
    ...
    const deleteSelected = () => {
        setMessages((prev) =>
            prev.map((m) => selected.includes(m.id) ? { ...m, isDeleted: true } : m)
        );
        setSelected([]);
    };
                            ...
                            paginated.map((msg) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 grid grid-cols-4 gap-3 items-center min-w-0">
                                        ...
                                        <p className="text-sm text-gray-400 truncate col-span-1">{msg.preview}</p>
                                        <p className="text-xs text-gray-400 text-right">{msg.date}</p>
                                    </div>
