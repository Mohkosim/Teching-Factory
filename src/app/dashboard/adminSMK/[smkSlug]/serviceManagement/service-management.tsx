...
export default function ServiceManagement({
    ...
    }, [services, search, jurusanFilter, statusFilter]);
    ...
    const handlePublikasi = async () => {
        ...
        startTransition(async () => {
            ...
            try {
                ...
                setServices((prev) =>
                    prev.map((s) =>
                        s.jasa_id === detailItem.jasa_id
                            ? { ...s, status_publikasi: "Published", catatan_revisi: null }
                            : s
                    )
                );
                Swal.close();
        ...
        startTransition(async () => {
            ...
            try {
                ...
                setServices((prev) =>
                    prev.map((s) =>
                        s.jasa_id === detailItem.jasa_id
                            ? { ...s, status_publikasi: "Revisi", catatan_revisi: revisiText }
                            : s
                    )
                );
                Swal.close();
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 p-5 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        <div className="flex flex-col gap-1">
                            ...
                            <Select
                                value={jurusanFilter}
                                onValueChange={(v) => { setJurusanFilter(v); setPage(1); }}
                            >
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-5">
                                ...
                                <div className="space-y-3">
                                    <div className="relative h-48 w-full rounded-xl overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center">
                                        {detailItem.fotos.length > 0 ? (
                                            // eslint-disable-next-line @next/next/no-img-element
                                            <img src={detailItem.fotos[activeImageIndex]} alt={detailItem.nama_jasa} className="h-full w-full object-cover" />
                                        ) : (
                            ...
                            <div className="flex items-center justify-between mt-5 pt-4 border-t border-gray-100">
                                <p className="text-xs text-gray-400">
                                    Estimasi : {detailItem.estimasi_pengerjaan ?? "-"} &nbsp;·&nbsp; Total Project : {detailItem.total_project} &nbsp;·&nbsp; Status : {detailItem.status_publikasi}
                                </p>
