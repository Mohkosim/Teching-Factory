...
export async function POST(req: NextRequest) {
    ...
    if (files.length > MAX_FILES) {
        return NextResponse.json({ message: `Maksimal ${MAX_FILES} foto per jasa` }, { status: 400 });
    }
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "jasa");
    ...
    for (const file of files) {
        ...
        urls.push(`/uploads/jasa/${filename}`);
    }
